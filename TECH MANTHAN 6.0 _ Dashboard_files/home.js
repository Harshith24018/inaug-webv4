...
function isTrioTeamEvent(eventId) {
  if (!eventId) return false;
...
function isDuoTeamEvent(eventId) {
  ...
  return (
    ...
    norm === "poster-making" ||
    norm === "postermaking" ||
    norm === "poster-designing" ||
    norm === "posterdesigning"
  );
}
...
window.registerEvent = async function (eventId) {
  ...
  // Special Handling for Trio 3-Member Team Event: IT Model & Marketing with PPT
  if (isTrioTeamEvent(eventId)) {
    openTrioTeamRegistrationModal(ev);
    return;
  }
...
function openTrioTeamRegistrationModal(ev) {
  const modal = document.getElementById("trioTeamModal");
  const modalClose = document.getElementById("trioTeamModalCloseBtn");
  const form = document.getElementById("trioTeamForm");
  const titleDisplay = document.getElementById("trioEventTitle");
  const iconDisplay = document.getElementById("trioEventIcon");
  const leaderNameDisplay = document.getElementById("trioLeaderNameDisplay");
  const leaderEmailDisplay = document.getElementById("trioLeaderEmailDisplay");
  ...
  if (titleDisplay) titleDisplay.innerText = `${(ev.title || "TRIO EVENT").toUpperCase()} REGISTRATION`;
  if (leaderNameDisplay) leaderNameDisplay.innerText = studentName;
  if (leaderEmailDisplay) leaderEmailDisplay.innerText = studentEmail;
  ...
  form.onsubmit = async (e) => {
    ...
    const teamName = document.getElementById("trioTeamName").value.trim();
    const member2 = document.getElementById("trioMember2").value.trim();
    const member3 = document.getElementById("trioMember3").value.trim();
    const member4 = document.getElementById("trioMember4") ? document.getElementById("trioMember4").value.trim() : "";

    if (!teamName || !member2 || !member3 || !member4) {
      alert("Please enter Team Name and all 4 Team Members' full names (all 4 members are required).");
      return;
    }

    const submitBtn = document.getElementById("btnSubmitTrioTeam");
    if (submitBtn) {
      submitBtn.disabled = true;
      submitBtn.innerText = "Registering Team...";
    }

    // Check Class Registration Limit for Team
    if (ev && currentStudentClass) {
      const classLimit = getClassRegistrationLimit(ev, currentStudentClass);
      if (classLimit !== null && classLimit > 0) {
        try {
          ...
          if (currentClassCount >= classLimit) {
            ...
            if (submitBtn) {
              submitBtn.disabled = false;
              submitBtn.innerText = "SUBMIT TEAM REGISTRATION (4 MEMBERS)";
            }
            return;
          }
        } catch (err) {
    ...
    const quadMembers = [studentName, member2, member3, member4];

    try {
      // 1. Save team in duoTeams, trioTeams & quadTeams collections for organizer access
      const teamDocId = `${username}_${ev.id}`;
      const teamRef = doc(db, "duoTeams", teamDocId);
      const teamData = {
        eventId: ev.id,
      ...
      const trioRef = doc(db, "trioTeams", teamDocId);
      await setDoc(trioRef, teamData);

      const quadRef = doc(db, "quadTeams", teamDocId);
      await setDoc(quadRef, teamData);

      // 2. Update student document registeredEvents & team data
      const studentRef = doc(db, "students", username);
      await setDoc(studentRef, {
        registeredEvents: arrayUnion(ev.id),
        [`duoTeam_${ev.id}`]: {
      ...
      // 3. Dispatch confirmation email to Team Leader
      sendTrioTeamEmail(ev, teamName, quadMembers);
...
async function sendTrioTeamEmail(ev, teamName, members) {
  const email = localStorage.getItem("email");
  const leaderName = localStorage.getItem("name") || username;
  if (!email) return;
    ...
    <div style="font-family: sans-serif; max-width: 600px; margin: 0 auto; border: 1px solid #e2e8f0; border-radius: 8px; overflow: hidden; background-color: #ffffff;">
      ...
      <div style="padding: 30px; color: #334155; line-height: 1.6;">
        ...
        <div style="margin: 25px 0; padding: 20px; background-color: #f8fafc; border-left: 4px solid #06b6d4; border-radius: 4px;">
          ...
          <div style="margin-top: 15px; border-top: 1px solid #cbd5e1; padding-top: 12px;">
            <strong style="color: #0f172a; display: block; margin-bottom: 6px;">👥 Team Roster (3 Members):</strong>
            <ul style="margin: 0; padding-left: 20px; color: #334155;">
              ${membersListHTML}
            </ul>
          </div>
        ...
        <p style="margin-top: 25px;">Please ensure all team members report to the event venue on time with college ID cards.</p>
  ...
  try {
    ...
    console.error("Error sending team confirmation email:", err);
  }
}
