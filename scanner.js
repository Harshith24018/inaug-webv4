...
document.addEventListener('DOMContentLoaded', () => {
    ...
    let fingertipParticles = []; // Local particle array
    ...
    function drawCyberHand(landmarks) {
        ...
        tipIndices.forEach(tipIndex => {
            const p = landmarks[tipIndex];
            const px = p.x * w;
            const py = p.y * h;
            ...
            if (Math.random() > 0.3) {
                fingertipParticles.push({
                    ...
                    vy: (Math.random() - 0.5) * 4 - 2, // drift up
                    life: 1,
                    color: tipIndex === 8 ? '#bc13fe' : (tipIndex === 4 ? '#39ff14' : '#00f3ff')
                });
            }
        });

        // Update and draw fingertip particles
        for (let i = fingertipParticles.length - 1; i >= 0; i--) {
            const fp = fingertipParticles[i];
            fp.x += fp.vx;
            fp.y += fp.vy;
            fp.life -= 0.05;
            
            if (fp.life <= 0) {
                fingertipParticles.splice(i, 1);
                continue;
            }

            ctx.beginPath();
            ctx.arc(fp.x, fp.y, Math.max(0.1, fp.life * 3.5), 0, Math.PI * 2);
    ...
    function updateProgressUI() {
        ...
        const psDetect = document.getElementById('ps-detect');
        const psAnalyze = document.getElementById('ps-analyze');
        const psVerify = document.getElementById('ps-verify');
        const psGrant = document.getElementById('ps-grant');

        if (psDetect && psAnalyze && psVerify && psGrant) {
            // Reset all
            [psDetect, psAnalyze, psVerify, psGrant].forEach(el => el.classList.remove('active', 'done'));

            if (rounded > 0 && rounded < 35) {
                psDetect.classList.add('active');
            } else if (rounded >= 35) psDetect.classList.add('done');

            if (rounded >= 35 && rounded < 70) {
                psAnalyze.classList.add('active');
            } else if (rounded >= 70) psAnalyze.classList.add('done');

            if (rounded >= 70 && rounded < 99) {
                psVerify.classList.add('active');
            } else if (rounded >= 99) psVerify.classList.add('done');

            if (rounded >= 100) {
                psGrant.classList.add('done');
            }
        }
    }
